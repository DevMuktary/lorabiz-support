  ...
  if (message.type === 'image' || message.type === 'document') {
    ...
    if (message.interactive.type === 'button_reply') {
      ...
      if (buttonId === 'agent_yes') {
       ...
       try {
          // PROPER PARSING AND BUG FIX HERE
          const responseJson = message.interactive.nfm_reply.response_json;
          console.log("[WHATSAPP FLOW] Raw payload received:", responseJson);
          flowData = JSON.parse(responseJson);
          
          const rawTopic = flowData.service_topic || '';
          const parsedTopic = rawTopic.includes('_') ? rawTopic.split('_').slice(1).join(' ') : rawTopic;
          content = `[Flow Submitted] Topic: ${parsedTopic || 'General Support'}`;
          ...
          content = '[Flow Data Received but processing failed]';
       }
    }
  } else {
